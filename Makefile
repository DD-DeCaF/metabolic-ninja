...
flake8:
	docker-compose run --rm web \
		flake8 src/metabolic_ninja tests

## Check Python package import order.
isort:
	docker-compose run --rm web \
		isort --check-only --recursive src/metabolic_ninja tests

## Sort imports and write changes to files.
isort-save:
	docker-compose run --rm web \
		isort --recursive src/metabolic_ninja tests

## Verify source code license headers.
license:
	./scripts/verify_license_headers.sh src/metabolic_ninja tests
